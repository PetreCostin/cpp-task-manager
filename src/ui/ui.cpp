            ...
            const std::string& statusMsg)
{
    ...
    if (rows < MIN_ROWS || cols < MIN_COLS) {
        clear();
        mvprintw(0, 0, "Terminal too small! Minimum: %d columns x %d rows.",
                 MIN_COLS, MIN_ROWS);
        refresh();
        return;
    }
    ...
    for (int i = 0; i < listH; i++) {
        ...
        const Task& t   = tm.tasks[static_cast<size_t>(idx)];
        ...
        if (static_cast<int>(tname.size()) > nW)
            tname = tname.substr(0, nW - 3) + "...";
        ...
        if (isSel)
            ...
            attron(COLOR_PAIR(CP_NORMAL));
        mvprintw(y, 18, " %-*s ", nW, tname.c_str());
        if (isSel)
